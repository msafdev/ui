      ...
      {years.map((year, index) => {
        ...
        return (
          <button
            ...
            className="relative inline-flex px-0.5 group/timeline"
            key={index}
          >
            <motion.div
            ...
            {isSelect && (
              <motion.code
                className={`absolute text-xs left-1/2 group-hover/timeline:-top-6 -top-5 -translate-x-1/2 text-foreground transition-all duration-200 ease-in-out`}
                initial={{ opacity: 0 }}
                animate={{ opacity: 0.6 }}
